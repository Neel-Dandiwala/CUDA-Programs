 ...
 * 
 * 
 * nvcc -I./ -I/usr/local/cuda/include -I/usr/local/cuda/lib64 -lcudart -lcuda --std c++17 *.cu -o streams_events.exe
 * **/
...
__host__ float * runStreamsFullAsync(float *host_mem, int num_elements)
{
    ...
    cudaEventCreate(&eventA1);
    cudaEventCreate(&eventA2);

    // Prepare device memory based on host memory
    float *dev_mem = allocateDeviceMemory(num_elements);
    copyFromDeviceToHostAsync(host_mem, dev_mem, num_elements, stream1);
    ...
    int s = 110;
    srand(s);
    float x = static_cast <float> (rand()) / (static_cast <float> (RAND_MAX/255.0f));
    kernelA1<<<blocksPerGrid, threadsPerBlock, 0, stream2>>>(dev_mem, num_elements, x);

    cudaEventRecord(eventA1, stream2);
    cudaStreamWaitEvent(stream3, eventA1, 0);
    kernelB1<<<blocksPerGrid, threadsPerBlock, 0, stream3>>>(dev_mem, num_elements);

    cudaEventRecord(eventA2, stream3);
    ...
    cudaStreamWaitEvent(stream5, eventA2, 0);
    kernelA2<<<blocksPerGrid, threadsPerBlock, 0, stream4>>>(dev_mem, num_elements, x);
...
__host__ float * runStreamsBlockingKernel2StreamsNaive(float *host_mem, int num_elements)
{
    ...
    cudaEventRecord(eventA2, stream3);
    cudaStreamWaitEvent(stream2, eventA2, 0);
    kernelA2<<<blocksPerGrid, threadsPerBlock, 0, stream2>>>(dev_mem, num_elements, x);
    kernelB2<<<blocksPerGrid, threadsPerBlock, 0, stream3>>>(dev_mem, num_elements);
