 ...
 * build = nvcc convertRGBToGrey.cu --std c++17 `pkg-config opencv --cflags --libs` -o convertRGBToGrey.exe -Wno-deprecated-gpu-targets $(shell pkg-config --cflags --libs opencv4) -I/usr/local/cuda/include -lcuda
 *
 */
__global__ void convert(uchar *d_r, uchar *d_g, uchar *d_b, uchar *d_gray)
{
    ...
    float sumOfSquares = pow(d_r[threadId],2) + pow(d_b[threadId],2) + pow(d_g[threadId],2);
    float averageValue = (sumOfSquares) / 3.0F;
    d_gray[threadId] = sqrt(averageValue);
